...
@section('content')
    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
            ...
            <div class="flex flex-wrap gap-2">
                <button class="px-4 py-2 bg-indigo-600 text-white rounded-xl text-sm font-bold shadow-md shadow-indigo-100 transition">
                    Semua Kategori
                </button>
                @foreach($categories as $category)
                    <button class="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-xl text-sm font-semibold hover:border-indigo-600 hover:text-indigo-600 transition shadow-sm">
                        {{ $category->name }}
                    </button>
                @endforeach
            </div>
    ...
    <section class="w-full bg-slate-50 border-t border-slate-200/60 py-16 block clear-both">
        <div class="max-w-7xl mx-auto px-6 text-center">
            <p class="text-xs font-extrabold uppercase tracking-widest text-slate-700 mb-8 block text-center w-full">
                Didukung Oleh Partner Resmi Kami
            </p>

            <div class="flex flex-wrap items-center justify-center gap-8">
                @forelse($partners as $partner)
                    <div class="flex flex-col items-center justify-center p-6 bg-white rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-md transition duration-300 w-44">
                        @if($partner->logo_url)
                            <img src="{{ asset($partner->logo_url) }}" alt="{{ $partner->name }}" class="h-12 w-auto object-contain transition duration-300">
                        @else
                            <div class="w-12 h-12 bg-indigo-50 text-indigo-600 font-bold flex items-center justify-center rounded-xl text-xs mb-1">
                                {{ strtoupper(substr($partner->name, 0, 2)) }}
                            </div>
                        @endif
                        <span class="text-xs font-bold text-slate-800 mt-3 tracking-tight text-center block w-full">{{ $partner->name }}</span>
                    </div>
                @empty
                    <div class="col-span-full text-center text-slate-400 text-sm italic py-4">
