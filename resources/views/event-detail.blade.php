        ...
        <div class="lg:col-span-1">
            <div class="sticky top-32">
                ...
                <img src="{{ $posterEvent ? asset('storage/' . $posterEvent) : asset('assets/concert.png') }}"
                     alt="{{ $event->title }}"
                     class="w-full rounded-[2.5rem] shadow-2xl border-8 border-white aspect-[3/4] object-cover">
                ...
                <div class="mt-8 p-6 bg-white rounded-3xl border border-slate-100 shadow-sm">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 font-bold">
                            {{ strtoupper(substr($event->organizer ?? 'AB', 0, 2)) }}
                        </div>
                        <div>
                            <p class="font-bold text-slate-800">{{ $event->organizer ?? 'ABP Productions' }}</p>
        ...
        <div class="lg:col-span-2 space-y-12">
            <div class="space-y-4">
                ...
                <span class="px-4 py-1.5 bg-indigo-100 text-indigo-700 rounded-full text-sm font-bold uppercase tracking-wider">
                    {{ $event->category->name ?? 'Event Festival' }}
                </span>
                ...
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                    <div>
                        ...
                        <p class="mt-4 text-indigo-100 flex items-center gap-2">
                            ...
                            @if($event->stock <= 0)
                                <span class="font-bold text-rose-300 underline">Mohon maaf, tiket sudah habis!</span>
                            @else
                    ...
                    <div>
                        ...
                        @else
                            <button disabled class="inline-block px-10 py-5 bg-slate-400 text-white rounded-2xl font-black text-xl cursor-not-allowed">
                                Habis
                            </button>
                        @endif
                    </div>
